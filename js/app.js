    ...
    (() => {
        ...
        function rangeInit() {
            ...
            if (rangeItems.length) rangeItems.forEach((rangeItem => {
                ...
                if (rangeItem.classList.contains("range-money")) {
                    ...
                    item.noUiSlider.on("update", (function(values, handle) {
                        toValue.value = Math.round(values[handle]) + " р";
                    }));
                }
                if (rangeItem.classList.contains("range-month")) {
                    nouislider.create(item, {
                        start: 6,
                        step: 6,
                        connect: "lower",
                    ...
                    item.noUiSlider.on("update", (function(values, handle) {
                        toValue.value = Math.round(values[handle]) + " месяцев";
                    }));
                }
            }));
        }
        ...
        function initSliders() {
            ...
            if (document.querySelector(".final__slider")) new core(".final__slider", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                spaceBetween: 30,
        ...
        formSubmit();
        pageNavigation();
        document.querySelector(".info-header__burger").addEventListener("click", (function openHeadMenu() {
            const menu = document.querySelector(".mob-menu");
            const menuInner = document.querySelector(".mob-menu__inner");
            menu.classList.toggle("_active");
            menuInner.classList.toggle("_active");
            document.querySelector(".info-header__burger").classList.toggle("_active");
            document.body.classList.toggle("lock");
        }));
        document.querySelector(".info-header__burger-mob").addEventListener("click", (function openHeadMenu() {
            const menu = document.querySelector(".mob-menu");
            const menuInner = document.querySelector(".mob-menu__inner");
            menu.classList.toggle("_active");
            menuInner.classList.toggle("_active");
            document.querySelector(".info-header__burger-mob").classList.toggle("_active");
            document.body.classList.toggle("lock");
        }));
        const em = document.querySelectorAll(".mob-menu__item");
        em.forEach((function(sv) {
            sv.addEventListener("click", (function openHeadMenu() {
                const menu = document.querySelector(".mob-menu");
                const menuInner = document.querySelector(".mob-menu__inner");
                menu.classList.toggle("_active");
                menuInner.classList.toggle("_active");
        ...
        datepicker(".form-test__date", {
            formatter: (input, date, instance) => {
                const value = date.toLocaleDateString();
                input.value = value;
            }
        });
        datepicker(".popup-test__date", {
            formatter: (input, date, instance) => {
                const value = date.toLocaleDateString();
                input.value = value;
            }
        });
        const svet = document.querySelectorAll(".form-test__svet-item");
        svet.forEach((function(sv) {
            sv.addEventListener("click", (function(e) {
                sv.classList.toggle("red");
            }));
        }));
        const app_formSubmit = document.querySelector(".form-test__button");
        const formControl = document.querySelector(".form-test__svet-control");
        app_formSubmit.addEventListener("click", (function(e) {
            for (var i = 0; i < svet.length; i++) if (false == svet[0].classList.contains("red")) if (true == svet[1].classList.contains("red")) if (false == svet[2].classList.contains("red")) if (false == svet[3].classList.contains("red")) formControl.checked = true; else formControl.checked = false; else formControl.checked = false; else formControl.checked = false; else formControl.checked = false;
        }));
